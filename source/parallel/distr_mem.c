...
void DistrMemory(unsigned w, unsigned h, unsigned frames, char* input_field_filename, char* output_field_filename, char* folder_name) {
	...
	for (unsigned int lo = 0; lo < frames; lo++) {
		...
		/*if (rank % 2) {
			MPI_Send(halo_top, pm.width, MPI_CHAR, prev_rank, 0, MPI_COMM_WORLD);
			MPI_Send(halo_bot, pm.width, MPI_CHAR, post_rank, 0, MPI_COMM_WORLD);
			MPI_Recv(recv_buffer_top, pm.width, MPI_CHAR, post_rank, 0, MPI_COMM_WORLD, MPI_STATUSES_IGNORE);
			MPI_Recv(recv_buffer_bot, pm.width, MPI_CHAR, prev_rank, 0, MPI_COMM_WORLD, MPI_STATUSES_IGNORE);
		}
		else {
			MPI_Recv(recv_buffer_top, pm.width, MPI_CHAR, post_rank, 0, MPI_COMM_WORLD, MPI_STATUSES_IGNORE);
			MPI_Recv(recv_buffer_bot, pm.width, MPI_CHAR, prev_rank, 0, MPI_COMM_WORLD, MPI_STATUSES_IGNORE);
